...
# dependencies = ["jax-md>=0.2.27", "lammps-jax"]
#
# [tool.uv.sources]
...
def build_fcc(n_cells, a=1.5, perturb=0.0):
    ...
    idx = jnp.arange(R_frac.shape[0], dtype=R_frac.dtype)
    R_frac = (R_frac + perturb * jnp.stack([jnp.sin(idx), jnp.cos(1.3 * idx), jnp.sin(0.7 * idx + 0.2)], 1)) % 1.0
...
def make_rd_energy_force(domain_energy, n_ranks, n_per):
    ...
    def sharded_energy_force(R_local, nbrs_local):
    ...
    def compute(R_mesh, nbrs_mesh):
        E, F = sharded_energy_force(R_mesh, nbrs_mesh)
...
def run_rd(cells, ranks, steps, warmup):
    ...
    for nc in cells:
        ...
        for n_ranks in sorted(ranks):
            ...
            elapsed = time_force(lambda: par_ef(R_mesh, nbrs_mesh)[1], warmup, steps)[1]
...
def run_eam(cells, ranks, steps, warmup):
    ...
    for nc in cells:
        ...
        for n_ranks in sorted(ranks):
            ...
            E_par.block_until_ready()
            F_par = F_par.block_until_ready()
            elapsed = time_force(lambda: par_ef(R)[1], warmup, steps)[1]
            e_err = float(jnp.abs(E_par - E_ref))
            f_err = float(jnp.max(jnp.abs(F_par - F_ref)))
...
def run_fd(cells, ranks, steps, warmup):
    n_devices = jax.local_device_count()
    ranks = ranks or [q * q for q in range(2, int(n_devices ** 0.5) + 1)]
    pair_fn = energy.multiplicative_isotropic_cutoff(energy.lennard_jones, r_onset=R_ONSET, r_cutoff=R_CUTOFF)
    summaries = []
    for nc in cells:
        R, box = build_fcc(nc, perturb=0.01 / nc)
        n_atoms = R.shape[0]
        displacement = space.periodic_general(box, fractional_coordinates=True)[0]
        domain_energy = replicate_data.domain_energy_from_pair(pair_fn, displacement)
        E_ref = domain_energy(R, 0, n_atoms).block_until_ready()
        F_ref_fn = jax.jit(lambda x: -grad(lambda r: domain_energy(r, 0, n_atoms))(x))
        F_ref, t_ref = time_force(lambda: F_ref_fn(R), warmup, steps)
        summaries.append(("fd", nc, "ref", n_atoms, float(E_ref), 0.0, 0.0, t_ref))
        for n_ranks in sorted(ranks):
            if n_ranks > n_devices or n_atoms % n_ranks != 0 or int(n_ranks ** 0.5) ** 2 != n_ranks:
                continue
            config = force_decomposition.create_config(n_atoms, n_ranks)
            par_ef = force_decomposition.make_sharded_energy_force(pair_fn, displacement, config)
            E_par, F_par = par_ef(R)
            E_par.block_until_ready()
            F_par = F_par.block_until_ready()
            elapsed = time_force(lambda: par_ef(R)[1], warmup, steps)[1]
            e_err = float(jnp.abs(E_par - E_ref))
            f_err = float(jnp.max(jnp.abs(F_par - F_ref)))
            summaries.append(("fd", nc, n_ranks, n_atoms, float(E_par), e_err, f_err, elapsed))
    return summaries
...
def run_unified(repeats, ranks, steps, warmup):
    ...
    for rep in repeats:
        ...
        for n_ranks in sorted(ranks):
            ...
            subgraph = feature_exchange.precompute_subgraphs(senders, receivers, shifts, mask, n_atoms, n_ranks, 1, jnp.zeros(n_atoms, dtype=jnp.int32))
            domain_node_idx, domain_senders, domain_receivers, domain_shifts, domain_species, domain_edge_mask = subgraph[:6]
            config = replicate_data.create_config(n_domains=n_ranks, n_atoms_per_domain=n_per)
            mesh = config.mesh
            ...
            def sharded_energy_force(R_local, node_idx, local_senders, local_receivers, local_shifts, local_species, edge_mask):
                R_all = lax.all_gather(R_local, axis_name=axis_name).reshape(-1, 3)
                node_idx = node_idx.squeeze(0)
                local_senders = local_senders.squeeze(0)
                local_receivers = local_receivers.squeeze(0)
                edge_mask = edge_mask.squeeze(0)
                pos_frac = R_all[node_idx]

                def lj_energy(edge_vectors):
                    dist_sq = jnp.sum(edge_vectors ** 2, axis=-1)
                    dist = jnp.sqrt(jnp.where(dist_sq > 0, dist_sq, 1.0))
                    valid = edge_mask & (dist_sq > 0)
                    e = jnp.where(valid, pair_fn(dist), 0.0)
                    owned = (local_receivers >= 0) & (local_receivers < n_per)
                    return 0.5 * jnp.sum(e * owned.astype(e.dtype))

                dr_frac = pos_frac[local_receivers] - pos_frac[local_senders]
            ...
            force_fn = jax.jit(sharded_energy_force)
            placed = [jax.device_put(array, sharding) for array in (
                R.reshape(n_ranks, n_per, 3), domain_node_idx, domain_senders, domain_receivers,
                domain_shifts, domain_species, domain_edge_mask)]
            E_par, F_par = force_fn(*placed)
            E_par = float(E_par.block_until_ready())
            F_par = F_par.reshape(-1, 3).block_until_ready()
            elapsed = time_force(lambda: force_fn(*placed)[1], warmup, steps)[1]
...
def run(mode="rd", sizes=None, ranks=None, steps=10, warmup=3):
    ...
    if mode == "fd":
        return run_fd(sizes or [4, 5], ranks, steps, warmup)
    raise ValueError(mode)


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--mode", choices=("rd", "unified", "eam", "fd"), default="rd")
